...
export default function ConnectedTeam() {
  ...
  return (
    <AnimatePresence mode='wait'>
      <div
        className='ml-[52px] flex justify-center h-screen items-center absolute top-0 left-0 z-90 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60'>
        <motion.div
        ...
        >
          {loading ? (
            <div
              ...
              <svg className="animate-spin opacity-20 h-3" viewBox="0 0 193 193" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <div className='flex gap-2'>
                        {team.heroes_selected &&
                          team.heroes_selected.map((hero) => (
                            <>
                              {hero.id ? (
                                <div key={hero.id} className='flex gap-2 w-12 h-12 overflow-hidden relative rounded-lg'>
                                  <Image
                                ...
                                <div key={hero.id} className='w-12 h-12 bg-gray-600 rounded-lg'></div>
