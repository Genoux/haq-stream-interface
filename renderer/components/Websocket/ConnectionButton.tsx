...
const ConnectionButton = ({ selectedRoom, disabled, className }: OBSConnectionProps) => {
  const { connectToOBS, disconnectOBS, obs, loading } = useOBS();
  const { toast } = useToast();

  const initiateConnection = async () => {
    if (selectedRoom) {
      const { error } = await connectToOBS(selectedRoom);
      if (error) {
        toast({
          variant: "destructive",
          title: error,
  ...
  return (
    <>
      <Button className={className} size='sm' onClick={initiateConnection} disabled={loading || disabled} variant={`${loading ? 'outline' : 'default'}`}>
        {loading ?
